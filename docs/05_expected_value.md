...
- 세그먼트 39개의 **성사율 순위와 효율(η) 순위의 상관은 0.669**다. 부트스트랩 90% 구간(0.616~0.692)이 모두 판정 경계 0.7 아래에 있다.
...
- 참조 인건비(57.6유로/시간)에서는 모든 세그먼트가 흑자다. 가장 불리한 세그먼트도 이자 중 순마진이 **7.95%** 이상이면 본전이다. 따라서 결론은 부호 불일치가 아니라 **효율 격차**다. 필요 순마진 비중은 세그먼트 간 **13.7배** 차이 나고, 이 배율은 인건비 가정과 무관하다.
...
|---|---|---|---|
| **수락 금액 × 작업시간 (주)** | **0.669** (90% 구간 0.616~0.692) | 0.454 | 0.219 |
| 금액×기간 × 작업시간 | 0.685 | 0.533 | 0.219 |
| 수락 금액 × 이벤트 수 | 0.596 | 0.454 | 0.661 |
| 금액×기간 × 이벤트 수 | 0.613 | 0.533 | 0.661 |

(구간은 2026-09-11 세그먼트 이름 변경 후 다시 계산한 값이다. 부트스트랩이 그룹 이름 순서대로 난수를 쓰기 때문에, 이름만 바뀌어도 구간이 0.609~0.691에서 0.616~0.692로 움직였다. 점추정과 판정은 같다. 기록: [`plans/ops-segment-labels.md`](plans/ops-segment-labels.md))

![성사율이 높은 세그먼트와 효율이 높은 세그먼트는 일치하지 않는다](../outputs/charts/p5_rate_vs_eta.png)

- **판정 (규칙 ①):** 주 조합 ρ = 0.669 < 0.7이고, 구간 상한(0.692)도 0.7 아래다 → **"다르다"**. 네 조합 모두 0.7 미만이다.
...
|---|---|---|---|---|---|---|
| **성사율은 높은데 효율은 낮음** | | | | | | |
| 6,500~10,000 · 한도 증액 | 369 | 0.694 | 9,170 | 13,235 | 6 | 25 |
| 10,000~15,500 · 신규·A_Submitted 없음 | 960 | 0.640 | 14,485 | 13,318 | 10 | 24 |
| 6,500~10,000 · 신규·A_Submitted 없음 · 기타 | 579 | 0.598 | 9,514 | 8,473 | 16 | 30 |
| 6,500 이하 · 신규·A_Submitted 없음 | 686 | 0.520 | 6,136 | 5,512 | 24 | 35 |
| 6,500~10,000 · 신규·A_Submitted 있음 · Home improvement | 1,015 | 0.570 | 9,820 | 10,024 | 18 | 28 |
| **성사율은 낮은데 효율은 높음** | | | | | | |
| 25,000 초과 · 신규·A_Submitted 있음 · 용도 불명 | 543 | 0.442 | 39,184 | 26,240 | 34 | 9 |
| 25,000 초과 · 신규·A_Submitted 있음 · Home improvement | 755 | 0.473 | 37,463 | 25,921 | 31 | 10 |
| 25,000 초과 · 신규·A_Submitted 있음 · Car | 336 | 0.497 | 32,334 | 21,885 | 28 | 11 |
| 25,000 초과 · 신규·A_Submitted 있음 · Existing loan takeover | 1,003 | 0.553 | 40,001 | 28,711 | 20 | 8 |
| 15,500~25,000 · 신규·A_Submitted 있음 · 용도 불명 | 477 | 0.497 | 21,776 | 16,796 | 29 | 19 |

- 성사율은 높지만 효율이 낮은 쪽은 **소액·중액**이다. 신규·A_Submitted 없음이나 한도 증액처럼 성사가 잘 되는 경로라도, 대출 규모가 작으면 공수 대비 효율이 낮다.
- 성사율은 낮지만 효율이 높은 쪽은 **25,000유로 초과의 신규·A_Submitted 있음**이다. 두 건 중 한 건 이상이 성사되지 않지만, 성사되면 규모가 크다.
...
![같은 공수를 효율 순서로 배분하면 대출 규모가 13% 늘어난다](../outputs/charts/p5_targeting_curve.png)
...
첫 세그먼트가 음수로 돌아서는 지점은 c/m = 4,374(6,500 이하 · 신규·A_Submitted 있음 · 용도 불명)다.
...
|---|---|---|
| 6,500 이하 · 신규·A_Submitted 있음 · Car | 724 | **7.95%** (최고) |
| 6,500 이하 · 신규·A_Submitted 있음 · 용도 불명 | 778 | 7.40% |
| 6,500 이하 · 신규·A_Submitted 있음 · Existing loan takeover | 872 | 6.60% |
| … | | |
| 금액 미기재 · 한도 증액 · 용도 불명 | 7,381 | 0.78% |
| 25,000 초과 · 한도 증액 | 9,858 | **0.58%** (최저) |

- **판정 (규칙 ⑤):** 가장 불리한 세그먼트도 순마진이 총이자의 7.95% 이상이면 흑자다. 이 비중이 현실적인지는 판정하지 않는다 (조달비용·신용손실 자료 없음, P6).
- **인건비 가정의 영향:** 참조값에는 간접비(시스템, 공간, 관리)가 빠져 있다. 실제 시간당 비용이 참조값의 k배라면 모든 세그먼트의 필요 비중도 k배가 된다. 그래도 **세그먼트 간 배율 13.7배는 변하지 않는다** (`14_targeting_and_scan.py` 출력).
- 따라서 이 프로젝트의 결론은 "어느 세그먼트가 손해인가"가 아니라 **"같은 공수가 세그먼트에 따라 13.7배 다른 가치를 만든다"**이다.

## 5. 반전 시나리오 점검 (CLAUDE.md §7)

| 시나리오 | 판정 | 근거 |
|---|---|---|
| 원 설계의 반전 — 성사율은 높은데 기대값이 음수인 구간 | **부호로는 미관측, 순위로는 관측** | 참조 인건비에서 전 세그먼트 흑자(§4). 대신 성사율 상위의 소액·중액 세그먼트가 η 하위에 있다(§2) |
