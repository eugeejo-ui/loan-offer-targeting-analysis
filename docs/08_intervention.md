...
Phase 1의 보유 주체 규칙으로 세그먼트마다 경과시간을 나눴다 (`outputs/p8_segment_holder.csv`).

| 항목 | 값 |
|---|---|
| 고객 보유 비중 ≥ 50%인 세그먼트 | **39 / 39** (`outputs/p8_intervention_checks.csv`) |
| 고객 보유 비중 범위 | 73.2%(25,000 초과 · 한도 증액) ~ 89.4%(6,500~10,000 · 신규·A_Submitted 없음 · Car) |
| 은행 보유 일수 중앙값 범위 | 0.93일(6,500~10,000 · 신규·A_Submitted 없음 · Car) ~ 5.18일(25,000 초과 · 신규·A_Submitted 있음 · Existing loan takeover) |

**판정:** 세그먼트 단위에서도 확인됐다. 은행 처리가 0초가 되더라도 어느 세그먼트에서든 경과시간의 대부분은 남는다. **처리 속도 개선을 단독 수단으로 권고하지 않는다.**

참고: ρ(η, 고객 보유 비중) = −0.309다. 효율이 높은 세그먼트일수록 은행이 쥐는 몫이 상대적으로 크다. 최상위 세그먼트(25,000 초과 · 한도 증액)는 26.8%다. 그러므로 속도는 **효율이 높은 세그먼트에 한해, 우선순위 조정과 결합될 때** 의미가 있다.

## 2. 현재 처리 순서는 효율을 반영하는가 (판정 2·3)

| 판정 | 값 | 결과 |
|---|---|---|
| ② ρ(세그먼트 η, 은행 보유 일수 중앙값) | **+0.346** | 현재 순서는 η를 반영하지 않는다. 방향은 오히려 반대다 → **순서 변경의 전제 성립** |
| ③ 은행 보유 시간 중 실제 작업 비중 (세그먼트 중앙값) | **0.61%** | 대기열 신호 (영업시간 미보정) |

(`outputs/p8_intervention_checks.csv`)

![효율이 높은 세그먼트일수록 은행 처리 대기시간이 증가한다](../outputs/charts/p8_eta_vs_bank_wait.png)

- **효율이 높은 세그먼트가 더 오래 기다린다.** 25,000유로 초과 대형 신청은 은행 보유 일수 중앙값이 2.9~5.2일이다. 1만 유로 이하 · 신규·A_Submitted 없음 세그먼트는 0.9~1.8일이다.
...
- **접수 경로에 따라 은행 보유 기간이 다르다.** 신규·A_Submitted 없음 세그먼트는 은행 보유 일수가 짧다(0.9~3.1일). 신규·A_Submitted 있음은 길다(2.4~5.2일). A_Submitted의 의미를 알 수 없으므로(Phase 1) 원인은 해석하지 않는다.
