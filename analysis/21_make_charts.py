...
from charts import (BASE, CHANNELS, GRAY, INK, INK2, MUTED, SERIES, SURFACE, channel_of, footnote,
                    headline, pretty_segment, save, setup)
...
def cancel_gap() -> str:
    ...
    ax.hist([days[system], days[~system]], bins=bins, stacked=True, rwidth=0.8,
            color=[SERIES[0], SERIES[1]], label=["시스템 계정 취소", "직원 취소"], zorder=3)
    ...
    ax.set_xlabel("마지막 오퍼 발송 후 취소까지 (일)")
    ax.set_ylabel("취소 건수")
    ax.yaxis.set_major_formatter(THOUSANDS)
    ax.set_xlim(0, 61)
    fig.legend(loc="upper right", bbox_to_anchor=(0.99, 0.88), ncol=2)
    headline(fig, "취소는 오퍼 발송 30일 뒤에 몰린다",
             "고객이 30일 동안 답하지 않으면 시스템이 취소한다 — 취소 케이스의 소요시간은 은행 처리가 아니라 이 규칙이 정한다")
    footnote(fig, "출처: outputs/p1_cancel_gap_summary.csv · 오퍼가 발송된 취소 9,593건 · 60일 이상은 60일 칸에 합산")
...
def rate_vs_eta() -> str:
    ...
    headline(fig, "성사율이 높다고 효율이 높은 것은 아니다",
             f"세그먼트 39개의 성사율 순위와 효율 순위의 상관 ρ = {align['rho_p_eta']:.2f} "
...
def targeting() -> str:
    ...
    for order, color, name in [("eta", SERIES[0], "효율(η) 높은 세그먼트부터"), ("p", SERIES[1], "성사율 높은 세그먼트부터")]:
    ...
    ax.annotate(f"{eta_v:,.1f}", (50, eta_v), xytext=(-8, 6), textcoords="offset points", ha="right", fontsize=9, color=INK2)
    ax.annotate(f"{p_v:,.1f}  (+{compare['eta_order_gain']:.1%} 차이)", (50, p_v), xytext=(8, -12),
    ...
    headline(fig, "같은 공수를 효율 순서로 쓰면 대출 규모가 13% 늘어난다",
             "공수 예산 50%에서 효율 순서 vs 성사율 순서 — 관측된 세그먼트 평균으로 계산한 가상 배분")
    footnote(fig, "출처: outputs/p5_targeting_compare.csv · 세그먼트 39개 · 개입 효과가 아니라 순서의 가치를 보여준다")
...
def multi_offer() -> str:
    ...
    headline(fig, "복수 오퍼의 우위는 '나중 상담'에서만 나온다",
             f"세그먼트 층화 후 단일 대비 성사율: 같은 상담 {same:+.1f}%p, 나중 상담 {later:+.1f}%p "
             "— 나중 상담은 고객이 남아 있어야 생기므로 상한이다")
    footnote(fig, "출처: outputs/p6_group_summary.csv, p6_stratified_diff.csv · 상담 구분 D1(생성 간격 1일 초과) · "
...
def failure_effort() -> str:
    ...
    ax.annotate("접촉 정책이 다룰 수 있는 몫", (before[1] + after[1] / 2, y[1] - 0.2), xytext=(0, -18),
    ...
    headline(fig, "실패한 신청이 공수의 34%를 쓰지만, 접촉 정책이 줄일 수 있는 몫은 3.5%",
             f"자동 취소 공수의 {auto_before:.0%}는 첫 오퍼 발송 전에 이미 들어갔다 — "
             f"거절 공수는 {denied_after:.0%}가 발송 뒤에 든다")
...
def eta_vs_bank_wait() -> str:
    ...
    ax.set_ylim(0, None)
    fig.legend(loc="upper right", bbox_to_anchor=(0.99, 0.86), ncol=3)
    headline(fig, "효율이 높은 세그먼트가 은행에서 더 오래 기다린다",
             f"ρ(효율, 은행 보유 일수) = {rho:+.2f} · 은행이 쥔 시간 중 실제 작업은 {active:.1%} — 나머지는 대기다")
...
def ab_duration() -> str:
    ...
    source = {"observed, stratified D1 (Phase 6)": "관측 D1", "observed, stratified D2 (Phase 6)": "관측 D2",
              "design minimum": "설계 최소", "design": "설계"}
    ...
    ax.barh(y[feasible], a.loc[feasible, "months_needed"], height=0.5, color=SERIES[0], label="6개월 안에 가능", zorder=3)
    ax.barh(y[~feasible], a.loc[~feasible, "months_needed"], height=0.5, color=GRAY, label="불가", zorder=3)
    ...
    headline(fig, "접촉 정책은 반년 안에 검증되지만, 첫 상담 단일 오퍼는 A/B로 검증할 수 없다",
             f"단일 오퍼 규칙이 실제로 바꾸는 신청은 월 {flows[True]:,}건, 접촉 정책 대상은 월 {flows[False]:,}건 "
             "(α 0.05 양측, 검정력 0.8, 1:1)")
    footnote(fig, "출처: outputs/p9_ab_sample_sizes.csv · 효과 크기는 추정치가 아닌 설계 파라미터 · "
